    ...
    private void selectAllMain() {
        ...
        if (!isSelectAll) {
            for (int i = 0, j = adapter.getMyLiveList().size(); i < j; i++) {
                adapter.getMyLiveList().get(i).setIschoosed(true);
                id=adapter.getMyLiveList().get(i).getGoods_id();
                list_id.add(id);
            }
...
//            ckAll.setText("取消全选");
            ...
            for (int i = 0, j = adapter.getMyLiveList().size(); i < j; i++) {
                adapter.getMyLiveList().get(i).setIschoosed(false);
                list_id.clear();
            }
            index = 0;
        ...
        sure.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                for (int i = adapter.getMyLiveList().size(), j =0 ; i > j; i--) {
                    GCollectionBean.ResultBean myLive = adapter.getMyLiveList().get(i-1);
...
//        map.put("limit", "12");
        ...
        Log.i("列表数据参数", Constant.BASE_URL + Constant.SHOUCANG + "?" + mapParam);
        MyApplication.getInstance().getMyOkHttp().post()
                .url(Constant.BASE_URL + Constant.SHOUCANG + "?" + mapParam)
                .tag(this)
...
//                                    String member_role = PreferUtils.getString(getContext(), "member_role");
                                    for (GCollectionBean.ResultBean listData : result) {
//                                        listData.setGoods_name();
